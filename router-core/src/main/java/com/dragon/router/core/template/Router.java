...
import com.dragon.router.core.Warehouse;
import com.dragon.router.core.callback.InterceptorCallback;
import com.dragon.router.core.callback.NavigationCallback;
import com.dragon.router.core.implments.InterceptorImpl;
import com.dragon.router.core.utils.ClassUtils;
import com.dragon.router_core.BuildConfig;
import com.dragon.router_core.R;
...
public class Router {
    ...
    private static void loadInfo() throws PackageManager.NameNotFoundException, InterruptedException, ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException, IOException {
        Set<String> routerMap = ClassUtils.getFileNameByPackageName(sApplication, Constants.PACKAGE_OF_GENERATE_FILE);
        for (String className : routerMap) {
            if (className.startsWith(Constants.PACKAGE_OF_GENERATE_FILE + "." + Constants.PROJECT + Constants.SEPARATOR + Constants.SUFFIX_ROOT)) {
                ((IRouteRoot) Class.forName(className).getConstructor().newInstance()).loadInto(Warehouse.routes);
            } else if (className.startsWith(Constants.PACKAGE_OF_GENERATE_FILE + "." + Constants.PROJECT + Constants.SEPARATOR + Constants.SUFFIX_INTERCEPTOR)) {
